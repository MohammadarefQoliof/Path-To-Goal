...
let titles = localStorage.getItem("sec2Titles");
if(!titles){
    localStorage.setItem("sec2Titles", JSON.stringify([]));
}
let mainDiv;
backtoplan.addEventListener("click", ()=>{
    window.location.href = "../index.html";
});
let secTitle = localStorage.getItem("title2")
let titleDiv = document.querySelector(".title")
titleDiv.textContent = secTitle
let sec2TaskNum = localStorage.getItem("sec2TaskNum");
if(!sec2TaskNum){
    localStorage.setItem("sec2TaskNum", "0");
}

if(sec2TaskNum <= 0 || !sec2TaskNum){
    ...
    for(let i = 1; i<=sec2TaskNum; i++){
        ...
        titleShow.textContent = JSON.parse(localStorage.getItem("sec2Titles"))[i-1]
        if(JSON.parse(localStorage.getItem("sec2Price"))[i-1] == ""){
            priceType.textContent = ""
        }else{
            priceType.textContent = `(${JSON.parse(localStorage.getItem("sec2Price"))[i-1]})`
            priceType.style.color = "grey"
        }
        if(JSON.parse(localStorage.getItem("sec2Date"))[i-1] == ""){
            dateType.textContent = ""
        }else{
            dateType.textContent = `(${JSON.parse(localStorage.getItem("sec2Date"))[i-1]})`
        ...
        bin.addEventListener("click", ()=>{
            let titlesList = JSON.parse(localStorage.getItem("sec2Titles")) || [];
            let priceList = JSON.parse(localStorage.getItem("sec2Price")) || [];
            let dateList = JSON.parse(localStorage.getItem("sec2Date")) || [];
            sec2TaskNum--;
            titlesList.splice(i-1, 1)
            priceList.splice(i-1, 1)
            dateList.splice(i-1, 1)
            localStorage.setItem("sec2TaskNum", sec2TaskNum)
            localStorage.setItem("sec2Titles", JSON.stringify(titlesList))
            localStorage.setItem("sec2Price", JSON.stringify(priceList))
            localStorage.setItem("sec2Date", JSON.stringify(dateList))
            location.reload()
        })
    ...
    localStorage.setItem("sec2Total", total)
}
...
function updateBar() {
    ...
    let sec2TaskNum = Number(localStorage.getItem("sec2TaskNum")) || 0
    let total = Number(localStorage.getItem("sec2Total")) || 0

    if (sec2TaskNum === 0) total = 0
    ...
    localStorage.setItem("sec2WalletInput", wallet)
    localStorage.setItem("sec2Percentage", percentage)
}

walletInput.addEventListener("input", updateBar)

window.addEventListener("DOMContentLoaded", () => {
    walletInput.value = localStorage.getItem("sec2WalletInput") || ""
    updateBar()
})
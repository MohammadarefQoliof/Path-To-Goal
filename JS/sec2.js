let title = localStorage.getItem("sec2 title")
let taskNum = Number(localStorage.getItem("sec2 task number"))
let mainDiv = document.querySelector("#main")
let h1 = document.createElement("h1")
h1.textContent = title
let time = localStorage.getItem("sec2 boolean time")
let price = localStorage.getItem("sec2 boolean price")
mainDiv.append(h1)
let button = document.createElement("div")
button.classList.add("savebutton")
button.textContent = "Save"

if(!localStorage.getItem("sec2 saved")){
    localStorage.setItem("sec2 saved", "false")
}
if(!localStorage.getItem("sec2 checked num")){
    localStorage.setItem("sec2 checked num", "0")
}
...
let prices = []
let times = []
if (!localStorage.getItem("sec2 checked")) {
    let checkedBoxes = [];
    for (let i = 0; i < taskNum; i++) {
        checkedBoxes.push(false);
    }
    localStorage.setItem("sec2 checked", JSON.stringify(checkedBoxes));
}
if(localStorage.getItem("sec2 saved") == "false"){
    for(let i = 1; i<=taskNum; i++){
        ...
        if(time == "true"){
            ...
            div.append(input)
            localStorage.setItem("sec2 times", JSON.stringify(times))
        }
        localStorage.setItem("sec2 tasks", JSON.stringify(tasks))
        mainDiv.append(div, button)
    }
}else{
    button.remove()
    let nameoftasks = JSON.parse(localStorage.getItem("sec2 tasks"));
    let myPrices = JSON.parse(localStorage.getItem("sec2 prices"));
    ...
    if(price == "true"){
        ...
        if(!localStorage.getItem("sec2 total")){
            localStorage.setItem("sec2 total", "0")
        }
        
        if(!localStorage.getItem("sec2 percentage")){
            localStorage.setItem("sec2 percentage", "0")
        }
        if(!localStorage.getItem("sec2 width")){
            localStorage.setItem("sec2 width", "0")
        }
        
        if(!localStorage.getItem("sec2 total sum")){
            localStorage.setItem("sec2 total sum", "false")
        }
        
        if(localStorage.getItem("sec2 total sum") == "false"){
            let total = Number(localStorage.getItem("sec2 total"))
            for(let i = 0; i < myPrices.length; i++){
                total += Number(myPrices[i])
                localStorage.setItem("sec2 total", total)
            }
            localStorage.setItem("sec2 total sum", "true")
        }
        total = localStorage.getItem("sec2 total")
        text.textContent = `Total: ${total} AZN`
        percent.textContent = `${localStorage.getItem("sec2 percentage")}%`
        completed.style.width = `${localStorage.getItem("sec2 width")}px`
        ...
        confrimButton.addEventListener("click", ()=>{
            if(isNaN(input.value) || input.value == ""){
                ...
                localStorage.setItem("sec2 percentage", fullPercent)
                localStorage.setItem("sec2 width", widthSize)
                completed.style.width = `${localStorage.getItem("sec2 width")}px`
                percent.textContent = `${localStorage.getItem("sec2 percentage")}%`
            }
            input.value = ""
...
button.addEventListener("click", ()=>{
    ...
    localStorage.setItem("sec2 tasks", JSON.stringify(tasks))
    localStorage.setItem("sec2 times", JSON.stringify(times))
    localStorage.setItem("sec2 prices", JSON.stringify(prices))
    localStorage.setItem("sec2 saved", "true")
    location.reload()
})
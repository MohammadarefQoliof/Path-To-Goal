...
let titles = localStorage.getItem("sec3Titles");
if(!titles){
    localStorage.setItem("sec3Titles", JSON.stringify([]));
}
let mainDiv;
backtoplan.addEventListener("click", ()=>{
    window.location.href = "../index.html";
});
let secTitle = localStorage.getItem("title3")
let titleDiv = document.querySelector(".title")
titleDiv.textContent = secTitle
let sec3TaskNum = localStorage.getItem("sec3TaskNum");
if(!sec3TaskNum){
    localStorage.setItem("sec3TaskNum", "0");
}

if(sec3TaskNum <= 0 || !sec3TaskNum){
    ...
    for(let i = 1; i<=sec3TaskNum; i++){
        ...
        titleShow.textContent = JSON.parse(localStorage.getItem("sec3Titles"))[i-1]
        if(JSON.parse(localStorage.getItem("sec3Price"))[i-1] == ""){
            priceType.textContent = ""
        }else{
            priceType.textContent = `(${JSON.parse(localStorage.getItem("sec3Price"))[i-1]})`
            priceType.style.color = "grey"
        }
        if(JSON.parse(localStorage.getItem("sec3Date"))[i-1] == ""){
            dateType.textContent = ""
        }else{
            dateType.textContent = `(${JSON.parse(localStorage.getItem("sec3Date"))[i-1]})`
        ...
        bin.addEventListener("click", ()=>{
            let titlesList = JSON.parse(localStorage.getItem("sec3Titles")) || [];
            let priceList = JSON.parse(localStorage.getItem("sec3Price")) || [];
            let dateList = JSON.parse(localStorage.getItem("sec3Date")) || [];
            sec3TaskNum--;
            titlesList.splice(i-1, 1)
            priceList.splice(i-1, 1)
            dateList.splice(i-1, 1)
            localStorage.setItem("sec3TaskNum", sec3TaskNum)
            localStorage.setItem("sec3Titles", JSON.stringify(titlesList))
            localStorage.setItem("sec3Price", JSON.stringify(priceList))
            localStorage.setItem("sec3Date", JSON.stringify(dateList))
            location.reload()
        })
    ...
    localStorage.setItem("sec3Total", total)
}
...
function updateBar() {
    ...
    let sec3TaskNum = Number(localStorage.getItem("sec3TaskNum")) || 0
    let total = Number(localStorage.getItem("sec3Total")) || 0

    if (sec3TaskNum === 0) total = 0
    ...
    localStorage.setItem("sec3WalletInput", wallet)
    localStorage.setItem("sec3Percentage", percentage)
}

walletInput.addEventListener("input", updateBar)

window.addEventListener("DOMContentLoaded", () => {
    walletInput.value = localStorage.getItem("sec3WalletInput") || ""
    updateBar()
})